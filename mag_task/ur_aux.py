...
class VrepModel:
  ...
  def getImage(self):
    ...
    newImg = cv2.cvtColor(newImg, cv2.COLOR_BGR2RGB) # RGB to BGR (in OpenCV style)
    # decrease image quality
    newImg = cv2.blur(newImg,(3,3))                  # smoothing    
    r,c,ch = newImg.shape
    rnd = np.random.rand(r,c,ch)                     # "salt and paper"
    prob = 0.01                                      #  noize threshold 
    newImg[rnd < prob] = 0
    newImg[rnd > (1-prob)] = 255
  ...
  def getToolOrientation(self):
    err,orient = vrep.simxGetObjectPosition(self.clientId,self.tipId,-1,vrep.simx_opmode_oneshot_wait)
